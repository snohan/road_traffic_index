...
For å finne variansen til trafikkarbeidsindeksen, kan vi ta utgangspunkt i formen på siste linje i @eq-w_indeks_2. Dette er et vektet aritmetisk gjennomsnitt av punktindeksene. Når vi har en begrenset populasjon, og vi vet størrelsen til denne, må vi ta hensyn til at punktene i utvalget ikke er uavhengige. SJEKK om utvalgsstørrelsen må være bestemt! Vi må derfor tenke oss at vi trekker ut punkter uten tilbakelegging. Hadde populasjonsstørrelsen vært uendelig stor, ville vi kunne si at trekkingen er tilnærmet med tilbakelegging. 
...
A = \sum_i^N a_iy_i.
$$
Denne har samme form som en et vektet, aritmetisk gjennomsnitt. For en trafikkindeks kan $a_i$ her spille rollen som trafikkarbeidsvekter. Hvis populasjonen kan antas å være homogen, det vil si at $y_i$ ikke kan stratifiseres ved hjelp andre variabler, så er den beste lineære, forventingsrette prediktoren for denne slik at 
...
$$
\widehat{\text{Var}}(\hat{A}) = \left[ \frac{1}{n} \left(\sum_r a_i \right)^2 + \sum_r a_i^2  \right] \hat{\sigma_s}^2.
$$
Dersom utvalget inkluderer hele populasjonen slik at $f = 1$, så vil $r = \emptyset$ og variansen blir null.

I en populasjon hvor alle $a_i = 1/N$, reduseres dette til

$$
\widehat{\text{Var}}(\hat{A}) = \hat{\sigma_s}^2 \frac{1-f}{n}.
$$
...
$$
med en ukjent, konstant parameter $\alpha$. Variansen $\sigma^2$ er også ukjent. Her antar vi at $p_{ai}$ er kjent. En forventingsrett estimator for variansen er i dette tilfellet
$$
\hat{\sigma}^2 = \frac{1}{n-1} \sum_i (p_{bi} - \hat{\alpha}p_{ai})^2.
$$
Vi må finne et uttrykk for $\hat{\alpha}$. Vi benytter maximum likelihood estimation (MLE) for en normalfordeling, og har da at vi skal minimere uttrykket 
$$
...
$$
\hat{\sigma}^2 = \widehat{\text{Var}}(p^b) = \frac{1}{n-1} \sum_i \left(p_{bi} - \frac{\sum_j p_{aj} p_{bj}}{\sum_j p_{aj}^2} p_{ai}\right)^2.
$$ {#eq-var_p_w_model}
...
Eller:
1. Pseudo-populasjon, varians
1. Reskalert bs, varians
1. Persentiler av bs-fordeling
