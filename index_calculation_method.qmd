...
$$
\hat{P}_{t,t+1} = \frac{\widehat{W}_{t+1}}{\widehat{W}_t} = \frac{\widehat{\bar{W}}_{t+1}}{\widehat{\bar{W}}_t}.
$$
...
$$
P_{\text{tw},ab} = \sum_{h} w_{h} P_{\text{tw},abh},
$$
hvor

$$
w_{h} = \frac{W_{h}}{\sum_h W_{h}},
$$
er andelen trafikkarbeid per funksjonsklasse.


### Forholdsestimator
Ved å multiplisere både teller og nevner i ligning @eq-w_indeks med $1/n$ kan indeksen også tolkes som forholdet mellom det gjennomsnittlige trafikkarbeidet i de to periodene. Den har da formen til en forholdsestimator. Indeksen i stratum $h$ er da

$$
P_h = \frac{ \bar{ W_{bh} } } { \bar{ W_{ah} } }. 
$$
Dersom trafikkarbeidet i referanseåret $W_{ah} \forall h$ antas kjent, så kan det totale trafikkarbeidet estimeres ved

$$
\widehat{W_b} = \sum_h \widehat{P_{abh}} W_{ah}.
$$
Indeksen blir da

$$
\widehat{P_{ab}} = \frac{\widehat{W_b}}{W_{a}} 
= \frac{1}{W_{a}} \sum_h \widehat{P_{abh}} W_{ah}
= \frac{1}{W_{a}} \sum_h \frac{ \bar{ W_{bh} } } { \bar{ W_{ah} } } W_{ah}
$$ {#eq-ratio_index}
hvor
...
W_a = \sum_h W_{ah}.
$$
Gjennomsnittlig trafikkarbeid per lenke estimeres da som i ligning @eq-W_bar_hat.

Ved små utvalg i ett eller flere strata, vil estimatoren kunne ha betydelig skjevhet. Det er derfor foreslått ulike måter å korrigere en forholdsestimator på for å redusere skjevhet. Beales estimator er god i så måte [@srivastva_1983], og er mye brukt.

Per stratum er en indeks med Beales korreksjon da

$$
\widehat{P_{abh}}_{,\text{Beale}} = 
\widehat{P_{abh}} \frac{1 + \left( \frac{1 - f_h}{n_h} \right) c_{abh} } { 1 + \left( \frac{1 - f_h}{n_h} \right) c_{aah} },
$$ {#eq-beale}
hvor

$$
\begin{aligned}
c_{abh} &= \frac{\rho_{abh} s_{ah} s_{bh}}{\bar{W_{ah}} \bar{W_{bh}}} \\
c_{aah} &= \frac{s_{ah}^2}{\bar{W_{ah}^2}}
\end{aligned}
$$
...
## Varians basert på utvalgsmetode
...
## Varians basert på populasjonsmodell
...
Her er vektene $w_{ai}$ ikke-stokastiske "pålitelighetsvekter" som reflekterer hvor viktige de enkelte punktindeksene er i beregningen.
...
$$
\widehat{\text{Var}}(P_{\text{tw},abh}) = 
\frac{1}{(\sum_k W_{akh})^2} \sum_i \frac{1}{1 - \frac{W_{aih} }{\sum_k W_{akh} } }
\left( W_{bih} - \frac{\sum_k W_{bkh} }{\sum_k W_{akh} } W_{aih}  \right)^2.
$$ {#eq-var_P_w_robust}

Variansen for samlet indeks over alle funksjonsklasser er da

$$
\widehat{\text{Var}}(P_{\text{tw},ab}) = \sum_h w_{ah}^2 \cdot \widehat{\text{Var}}(P_{\text{tw},abh}).
...
## Forholdsestimator
Ligning @eq-beale gir oss en skjevhetskorrigert indeks. Den samlede variansen til denne indeksen består av et bidrag fra variansen til ukorrigert form i ligning @eq-ratio_index og et bidrag fra en systematisk feil fra små utvalg

$$
\text{MSE}(\widehat{P_{ab}}) = 
\frac{1}{W_a^2} \sum_h \left( \text{Var}(\bar{W_b}) + W_{ah}^2 (\mathbb{E}(\widehat{P_{abh}} - P_{abh}))^2 \right)
$$
Her er 

$$
\text{Var}(\bar{W_b}) = \sum_h \frac{N_h^2(1 - f_h)}{n_h} S_{dh}^2
$$
med

$$
S_{dh}^2 = \frac{1}{N-1} \sum_i (W_{bi} - P_{ab} W_{ai})^2 =
s_a^2 + P_{ab}^2 s_x^2 - 2 \rho_{ab} s_a s_b.
$$
...
\frac{1 - f_h}{n_h \bar{W_a^2}} \left(
P_h s_{ah}^2 - \rho_{ab} s_{ah} s_{bh}
\right).
$$
